...
public class EdgeWeightedDigraph {
    ...
    private Map<Integer, List<DirectedEdge>> adj = new LinkedHashMap<>();
    ...
    public EdgeWeightedDigraph(int V, InputStream inputStream) throws IOException {
        this(V);
        int lineCount = 0;
        try (BufferedReader br = new BufferedReader(new InputStreamReader(inputStream))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                // example input:      3	2,1	4,1
                String[] values = line.replaceAll("\\s", " ").trim().split(" ");
                if (values.length == 0) {
                ...
                if (parsedValues.isEmpty()) {
                    System.err.println("Skipping empty parsedValues list on line " + lineCount);
                    continue;
                }

                // read source vertex
                final int s = Integer.valueOf(parsedValues.get(0));

                // read remaining pairs (v,w) where v is always s
                for (int i = 1; i < parsedValues.size(); i++) {
                    String[] data = parsedValues.get(i).split(",");
                    addUndirectedEdge(s, Integer.parseInt(data[0]), Integer.parseInt(data[1]));
                }
            }
        }

        System.out.println("Initialized Digraph with " + V + " vertices and " + E + " edges. ");
        System.out.println("Input data file line count " + lineCount);
    }
    ...
    public void addEdge(DirectedEdge e) {
        ...
        List<DirectedEdge> list = adj(v);
        list.add(0, e); // remove 0 for reversed order
        adj.put(v, list);
    ...
    public void addUndirectedEdge(int v, int w, int weight) {
        ...
        if (E < 150) {
            System.out.println("Add edge " + v + "->" + w + " of weight " + weight);
        } else if (E == 150) {
            System.out.println("..........");
        }

        // transform to 0-based array indexing
        v--;
        w--;
    ...
    public List<DirectedEdge> adj(int v) {
        List<DirectedEdge> res = adj.get(v);
        if (res == null) {
            adj.put(v, res = new LinkedList<>());
        }
        return res;
    }
